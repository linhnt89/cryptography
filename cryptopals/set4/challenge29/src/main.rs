...
fn main() {
    ...
    loop {
        ...
        let new_mac = sha::SHA1::hash(
            new_text, &mac, padded_msg.len() + key_len);
...
fn get_mac(key: &[u8], msg: &[u8]) -> Vec<u32> {
...
fn find_padding(key_len: usize, msg: &[u8]) -> Vec<u8> {
    ...
    let mut padded_l = (keyed_msg_len as u64).to_be_bytes().to_vec();
