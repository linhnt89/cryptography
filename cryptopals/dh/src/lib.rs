...
const FFDHE2048_P : &str = "FFFFFFFFFFFFFFFFADF85458A2BB4A9AAFDC5620273D3CF1\
...
// const FFDHE2048_Q : &str = "7FFFFFFFFFFFFFFFD6FC2A2C515DA54D57EE2B10139E9E78\
...
const FFDHE3072_P : &str = "FFFFFFFFFFFFFFFFADF85458A2BB4A9AAFDC5620273D3CF1\
                            D8B9C583CE2D3695A9E13641146433FBCC939DCE249B3EF9\
                            7D2FE363630C75D8F681B202AEC4617AD3DF1ED5D5FD6561\
                            2433F51F5F066ED0856365553DED1AF3B557135E7F57C935\
                            984F0C70E0E68B77E2A689DAF3EFE8721DF158A136ADE735\
...
// const FFDHE3072_Q : &str = "7FFFFFFFFFFFFFFFD6FC2A2C515DA54D57EE2B10139E9E78\
//                             EC5CE2C1E7169B4AD4F09B208A3219FDE649CEE7124D9F7C\
//                             BE97F1B1B1863AEC7B40D901576230BD69EF8F6AEAFEB2B0\
//                             9219FA8FAF83376842B1B2AA9EF68D79DAAB89AF3FABE49A\
//                             CC278638707345BBF15344ED79F7F4390EF8AC509B56F39A\
...
const FFDHE4096_P : &str = "FFFFFFFFFFFFFFFFADF85458A2BB4A9AAFDC5620273D3CF1\
                            D8B9C583CE2D3695A9E13641146433FBCC939DCE249B3EF9\
                            7D2FE363630C75D8F681B202AEC4617AD3DF1ED5D5FD6561\
                            2433F51F5F066ED0856365553DED1AF3B557135E7F57C935\
                            984F0C70E0E68B77E2A689DAF3EFE8721DF158A136ADE735\
                            ...
                            FFFFFFFFFFFFFFFF";
// const FFDHE4096_Q : &str = "7FFFFFFFFFFFFFFFD6FC2A2C515DA54D57EE2B10139E9E78\
//                             EC5CE2C1E7169B4AD4F09B208A3219FDE649CEE7124D9F7C\
//                             BE97F1B1B1863AEC7B40D901576230BD69EF8F6AEAFEB2B0\
//                             9219FA8FAF83376842B1B2AA9EF68D79DAAB89AF3FABE49A\
//                             CC278638707345BBF15344ED79F7F4390EF8AC509B56F39A\
...
//                             7FFFFFFFFFFFFFFF";
const FFDHE6144_P : &str = "FFFFFFFFFFFFFFFFADF85458A2BB4A9AAFDC5620273D3CF1\
                            D8B9C583CE2D3695A9E13641146433FBCC939DCE249B3EF9\
                            7D2FE363630C75D8F681B202AEC4617AD3DF1ED5D5FD6561\
                            2433F51F5F066ED0856365553DED1AF3B557135E7F57C935\
                            984F0C70E0E68B77E2A689DAF3EFE8721DF158A136ADE735\
...
// const FFDHE6144_Q : &str = "7FFFFFFFFFFFFFFFD6FC2A2C515DA54D57EE2B10139E9E78\
//                             EC5CE2C1E7169B4AD4F09B208A3219FDE649CEE7124D9F7C\
//                             BE97F1B1B1863AEC7B40D901576230BD69EF8F6AEAFEB2B0\
//                             9219FA8FAF83376842B1B2AA9EF68D79DAAB89AF3FABE49A\
//                             CC278638707345BBF15344ED79F7F4390EF8AC509B56F39A\
...
const FFDHE8192_P : &str = "FFFFFFFFFFFFFFFFADF85458A2BB4A9AAFDC5620273D3CF1\
                            D8B9C583CE2D3695A9E13641146433FBCC939DCE249B3EF9\
                            7D2FE363630C75D8F681B202AEC4617AD3DF1ED5D5FD6561\
                            2433F51F5F066ED0856365553DED1AF3B557135E7F57C935\
                            984F0C70E0E68B77E2A689DAF3EFE8721DF158A136ADE735\
...
// const FFDHE8192_Q : &str = "7FFFFFFFFFFFFFFFD6FC2A2C515DA54D57EE2B10139E9E78\
//                             EC5CE2C1E7169B4AD4F09B208A3219FDE649CEE7124D9F7C\
//                             BE97F1B1B1863AEC7B40D901576230BD69EF8F6AEAFEB2B0\
//                             9219FA8FAF83376842B1B2AA9EF68D79DAAB89AF3FABE49A\
//                             CC278638707345BBF15344ED79F7F4390EF8AC509B56F39A\
...
pub fn get_p_g(mode: DHMODE) -> (UBig, UBig) {
    let p = match mode {
        DHMODE::FFDHE2048 => {
            UBig::from_str_radix(FFDHE2048_P, 16).unwrap()
        },
        DHMODE::FFDHE3072 => {
            UBig::from_str_radix(FFDHE3072_P, 16).unwrap()
        },
        DHMODE::FFDHE4096 => {
            UBig::from_str_radix(FFDHE4096_P, 16).unwrap()
        },
        DHMODE::FFDHE6144 => {
            UBig::from_str_radix(FFDHE6144_P, 16).unwrap()
        },
        DHMODE::FFDHE8192 => {
            UBig::from_str_radix(FFDHE8192_P, 16).unwrap()
        },
    };
    let g = ubig!(2);
    (p, g)
...
pub fn modular_pow(mut b: UBig, mut e: UBig, m: UBig) -> UBig {
    let mut ret = ubig!(0);
    let ring = ModuloRing::new(&m);
    if m != ubig!(1) {
        ret = ubig!(1);
        b = ring.from(b).residue();
        while e > ubig!(0) {
            if e.bit(0) == true {
                ret = ring.from(ret * &b).residue();
            }
            e >>= 1;
            b = ring.from(b.pow(2)).residue();
        }
    }
    ret
}

pub fn get_keypair(mode: DHMODE) -> (UBig, UBig) {
    let (p, min, max) = match mode {
        DHMODE::FFDHE2048 => {
            let p = UBig::from_str_radix(FFDHE2048_P, 16).unwrap();
            let min = ubig!(2).pow(224);
            let max = ubig!(2).pow(225);
            (p, min, max)
        },
        DHMODE::FFDHE3072 => {
            let p = UBig::from_str_radix(FFDHE3072_P, 16).unwrap();
            let min = ubig!(2).pow(274);
            let max = ubig!(2).pow(275);
            (p, min, max)
        },
        DHMODE::FFDHE4096 => {
            let p = UBig::from_str_radix(FFDHE4096_P, 16).unwrap();
            let min = ubig!(2).pow(324);
            let max = ubig!(2).pow(325);
            (p, min, max)
        },
        DHMODE::FFDHE6144 => {
            let p = UBig::from_str_radix(FFDHE6144_P, 16).unwrap();
            let min = ubig!(2).pow(374);
            let max = ubig!(2).pow(375);
            (p, min, max)
        },
        DHMODE::FFDHE8192 => {
            let p = UBig::from_str_radix(FFDHE8192_P, 16).unwrap();
            let min = ubig!(2).pow(399);
            let max = ubig!(2).pow(400);
            (p, min, max)
        },
    };
    let g = ubig!(2);
    let prk = thread_rng().gen_range(min..max);
    let puk = modular_pow(g, prk.clone(), p);
    (prk, puk)
}
...
mod tests {
    ...
    #[test]
    fn test_get_keypair() {
        let (prka, puka) = get_keypair(DHMODE::FFDHE2048);
        let (prkb, pukb) = get_keypair(DHMODE::FFDHE2048);
        let p = UBig::from_str_radix(FFDHE2048_P, 16).unwrap();
        let sa = modular_pow(pukb, prka, p.clone());
        let sb = modular_pow(puka, prkb, p);
        assert_eq!(sa, sb);

        let (prka, puka) = get_keypair(DHMODE::FFDHE3072);
        let (prkb, pukb) = get_keypair(DHMODE::FFDHE3072);
        let p = UBig::from_str_radix(FFDHE3072_P, 16).unwrap();
        let sa = modular_pow(pukb, prka, p.clone());
        let sb = modular_pow(puka, prkb, p);
        assert_eq!(sa, sb);

        let (prka, puka) = get_keypair(DHMODE::FFDHE4096);
        let (prkb, pukb) = get_keypair(DHMODE::FFDHE4096);
        let p = UBig::from_str_radix(FFDHE4096_P, 16).unwrap();
        let sa = modular_pow(pukb, prka, p.clone());
        let sb = modular_pow(puka, prkb, p);
        assert_eq!(sa, sb);

        let (prka, puka) = get_keypair(DHMODE::FFDHE6144);
        let (prkb, pukb) = get_keypair(DHMODE::FFDHE6144);
        let p = UBig::from_str_radix(FFDHE6144_P, 16).unwrap();
        let sa = modular_pow(pukb, prka, p.clone());
        let sb = modular_pow(puka, prkb, p);
        assert_eq!(sa, sb);

        let (prka, puka) = get_keypair(DHMODE::FFDHE8192);
        let (prkb, pukb) = get_keypair(DHMODE::FFDHE8192);
        let p = UBig::from_str_radix(FFDHE8192_P, 16).unwrap();
        let sa = modular_pow(pukb, prka, p.clone());
        let sb = modular_pow(puka, prkb, p);
        assert_eq!(sa, sb);
